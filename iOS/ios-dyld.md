...
`dyld`包含在`NeXTStep 3.3`中，而`NeXTStep`可以追溯到`1996`年，此前`NeXT`使用静态二进制数据。`NeXTStep`具有不同的专用扩展，所以苹果开发者在`macOS 10`的早起版本上编写第三方包装器，以支持标准`Unix`软件。但是还是不能完全的匹配，经常有边界例子不能正常工作。因此造成运行缓慢。还有就是这个第三方库大规模的使用`C++`库。也造成了运行缓慢。
