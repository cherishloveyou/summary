## RTSP
 ...
 RTSP(Real-Time Stream Protocol)是一种**基于文本的多媒体播放控制协议，属于应用层协议**。利用RTSP可以在服务器和客户端之间建立并控制连续的音频媒体和视频媒体流，进行服务器和客户端之间的“网络远程控制”，提供音频和视频的远程控制功能。但是RTSP要在独立于数据的通道中进行传输。RTSP支持单播和组播，提供选择传送通道的方法，可以选择UDP、组播UDP和TCP，而低层的传输机制依赖于RTP或TCP。RTSP与低层的协议（如RTP、RSVP）一起协调运行，提供完全的流服务。RTSP是有状态的、对称的协议，RTSP服务器维护会话状态且RTSP中媒体服务器和客户端都能发出请求。RTSP的语法和操作与HTTP相似，使用专用于音频和视频的类似于HTTP中的URL。

和HTTP类似，使用TCP传输，RTSP主要完成媒体信息交互，确认媒体传输方式和媒体传输控制（即RTSP负责上层控制），底层音视频传输使用的是其他的协议（RTP协议）。RTSP交互过程中会把媒体信息告知对端，这个媒体信息就放在RTSP的payload（负载）中，和HTTP中的payload是一样的，在RTSP中媒体信息，也就是这个payload使用的是SDP协议，SDP协议就是规定了怎么描述媒体源(网络摄像头)包含哪些媒体流(主要是音频和视频)以及这些媒体流参数是怎么样的（音视频编解码参数等）。所以从IPC（网络摄像头）中获取音视频涉主要及到三个协议：RTSP、SDP、RTP。RTP/RTCP是基于IP的应用层协议。
...
RTCP 提供数据分发质量反馈信息，这是 RTP 作为[传输协议](https://baike.baidu.com/item/传输协议/0?fromModule=lemma_inlink)的部分功能并且它涉及到了其它传输协议的[流控制](https://baike.baidu.com/item/流控制/0?fromModule=lemma_inlink)和[拥塞控制](https://baike.baidu.com/item/拥塞控制/0?fromModule=lemma_inlink)。

RTCP 为 RTP 源携带一个持久性[传输层](https://baike.baidu.com/item/传输层/0?fromModule=lemma_inlink)[标识符](https://baike.baidu.com/item/标识符/0?fromModule=lemma_inlink)，称为规范名或 CNAME。
...
|TEARDOWN      | C->S	   |是	|TEARDOWN请求停止给定URL流发送，释放相关资源。|ß
...
##### 3、RTP OVER TCP/RTSP
  ...
  要使用TCP连接，RTSP客户端需要在SETUP阶段请求TCP连接。SETUP命令中应该包括如下格式的Transport：Transport: RTP/AVP/TCP;interleaved=0-1
...
- **RTSP 的主要功能和用途是什么？RTSP 与 RTP 的关系是什么？**

**RTSP (Real-Time Streaming Protocol)** 和 **RTP (Real-Time Protocol)** 两者都是用于流媒体的协议，但它们的作用和功能有所不同。以下是它们之间关系的概述：

###### 1. 功能差异

- **RTSP** 是一个控制协议，主要用于建立和控制流媒体会话。它用于发送请求和命令，例如开始播放（PLAY）、暂停（PAUSE）和停止（TEARDOWN），以管理媒体流的传输。
- **RTP** 是一种传输协议，专门用于在网络上实时传输音频和视频数据。它负责封装和传送媒体数据包，并提供时间戳、序列号等信息，以确保流媒体的顺序和同步。

###### 2. 配合使用

- RTSP 通常与 RTP 配合使用。在一个典型的配置中，RTSP 负责初始化和控制会话，而 RTP 则负责实际的数据传输。例如，RTSP 会在客户端和服务器之间协商要使用哪些 RTP 流来传输媒体内容。

###### 3. 会话管理

- RTSP 提供了会话管理的能力，允许客户端和服务器在同一会话中进行多个操作，例如选择不同的媒体流、控制播放参数等，而 RTP 则不具备这些控制功能。

###### 4. 协议层次

- RTP 通常基于 UDP（用户数据报协议）进行工作，这有助于实现低延迟的媒体传输。这意味着 RTP 会处理数据包的快速传送，而 RTSP 则负责更高层次的控制和管理。

###### 总结

RTSP 和 RTP 是互补的，RTSP 负责控制和管理流媒体会话，而 RTP 负责实际传输媒体数据。在实时流媒体应用中，两者通常会同时使用，以实现流的高效传输和控制

- **RTSP 的请求方法（如 PLAY、PAUSE、SETUP）及其作用是什么？**

### 2. Sticky Headers 的概念

- **“Sticky Headers” 是什么？在 RTSP 中如何使用？**
- **Sticky Headers 如何影响流媒体的持续性和连接的稳定性？**
- **在 RTSP 中实现 Sticky Headers 的挑战和注意事项是什么？**

### 3. RTSP 的实现和优化

- **如何优化 RTSP 流的性能？**
- **在 RTSP 服务器和客户端之间，如何实现有效的连接管理？**
- **在处理流媒体时，如何保证低延迟和高可靠性？**

### 4. 相关协议与技术

- **RTSP 与 HTTP 的区别和联系是什么？**
- **什么是 SDP（Session Description Protocol）？它在 RTSP 中的角色是什么？**

### 5. 实际应用和经验

- **你在使用 RTSP 进行流媒体传输时遇到过什么问题？如何解决的？**
- **在开发过程中，你如何处理 RTSP 流的错误和异常？**

### 6. 安全与难点

- **RTSP 中的安全问题（如认证与加密）有哪些？**
- **如何增强 RTSP 流媒体传输的安全性？**
